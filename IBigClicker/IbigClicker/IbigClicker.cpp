...
int main() {
    std::cout <<
        "=== СУПЕР АЛЬФА - КЛИКЕР ===\n"
        "Ctrl + Shift + 1 - установить координату 1\n"
        "Ctrl + Shift + 2 - установить координату 2\n"
        "Ctrl + Shift + 3 - режим: клика по 2 точкам\n"
        "Ctrl + Shift + 4 - режим: координата 1 + пробел\n"
        "Ctrl + Shift + 5 - остановить АЛЬФА - КЛИКЕР\n\n";
    ...
    while (GetMessage(&msg, nullptr, 0, 0)) {
        if (msg.message == WM_HOTKEY) {
            ...
            case 1:
                GetCursorPos(&p1);
                savePoint("P1", p1);
                std::cout << "P1 saved: " << p1.x << " " << p1.y << "\n";
                break;

            case 2:
                GetCursorPos(&p2);
                savePoint("P2", p2);
                std::cout << "P2 saved: " << p2.x << " " << p2.y << "\n";
                break;

            case 3:
                running = true;
                mode = 3;
                std::cout << "Mode: two points\n";
                break;

            case 4:
                running = true;
                mode = 4;
                std::cout << "Mode: point + space\n";
                break;
